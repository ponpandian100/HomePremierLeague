            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                ...
                <div style={{ fontFamily: "'Bebas Neue',sans-serif", lineHeight: 0.88 }}>
                  <div style={{ fontSize: 38, color: "rgba(255,255,255,0.9)", letterSpacing: "0.02em" }}>HIGHEST</div>
                  <div className="shine-title" style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 54, letterSpacing: "0.02em" }}>FPTS</div>
                </div>
              </div>
            </div>
          </div>

          {/* ── Top 5 Highest Scores Section ── */}
          {topScores.length > 0 && (
            <>
              {/* Divider */}
              <div style={{ margin: "20px 0 15px", display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ flex: 1, height: 1, background: "linear-gradient(90deg, transparent, rgba(255,255,255,0.08))" }} />
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", color: "rgba(255,255,255,0.2)", textTransform: "uppercase", whiteSpace: "nowrap" }}>
              ...
              {/* Sub-label */}
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.15)", fontWeight: 600, letterSpacing: "0.12em", marginBottom: 14, textTransform: "uppercase" }}>
                Best individual match performances
              </div>

              <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                {topScores.map((entry, i) => (
                  <HighScoreRow key={`${entry.player}-${entry.matchNum}-${i}`} entry={entry} rank={i + 1} delay={0.55 + i * 0.07} />
                ))}
              </div>
            </>
          )}
